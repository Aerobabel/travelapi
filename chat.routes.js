...
const generativeModel = genAI ? genAI.getGenerativeModel({ model: "gemini-1.0-pro-latest" }) : null;
...
// (getMem, updateProfileFromHistory, extractDestination, and pickPhoto are unchanged)
const getMem = (userId) => {
...
function updateProfileFromHistory(messages, mem) {
  ...
  const mappings = {
    ...
    travel_alone_or_with: { solo: /solo|by myself/, family: /family|with my kids/, friends: /friends|group/, }, "flight_preferences.class": { premium_economy: /premium economy/, business: /business class/, first: /first class/, }, "budget.prefer_comfort_or_saving": { comfort: /comfort|luxury/, saving: /saving|budget/ },
  ...
  for (const key in mappings) {
    for (const value in mappings[key]) {
      if (mappings[key][value].test(userTexts)) {
        if (key.includes(".")) { const [p, c] = key.split("."); profile[p][c] = value; }
        else if (Array.isArray(profile[key])) { if (!profile[key].includes(value)) profile[key].push(value); }
...
async function pickPhoto(dest, reqId) {
  ...
  if (imageCache.has(cacheKey)) { logInfo(reqId, `[CACHE HIT] Serving image for "${dest}"`); return imageCache.get(cacheKey); }
  logInfo(reqId, `[CACHE MISS] Fetching new image for "${dest}"`);
  if (!UNSPLASH_ACCESS_KEY) { logError(reqId, "UNSPLASH_ACCESS_KEY is not set."); return FALLBACK_IMAGE_URL; }
  ...
  try {
    ...
    if (!res.ok) { logError(reqId, `Unsplash API error: ${res.status}`); return FALLBACK_IMAGE_URL; }
  ...
  } catch (e) { logError(reqId, "Failed to fetch from Unsplash API", e.message); return FALLBACK_IMAGE_URL; }
}
...
// --- NEW: GEMINI TOOL DEFINITIONS ---
const toolDeclarations = [
  {
    functionDeclarations: [
      { name: "request_dates", description: "Call this to ask the user for their travel dates." },
      { name: "request_guests", description: "Call this to ask the user how many people are traveling." },
      {
        name: "search_flights",
      ...
      {
        name: "create_plan",
        description: "Call this ONLY when all information (flights, hotels, dates, etc.) has been gathered. It creates the final travel plan for the user.",
        parameters: {
          type: "object",
          properties: { /* Same properties as your original create_plan tool */
            ...
            costBreakdown: { type: "array", items: { type: "object", properties: { item: { type: "string" }, provider: { type: "string" }, details: { type: "string" }, price: { type: "number" }, iconType: { type: "string" }, iconValue: { type: "string" } } } },
          },
...
function normalizeToGemini(messages = []) {
  ...
  if (firstUserMsg) {
      history.push({ role: 'user', parts: [{ text: firstUserMsg.content || firstUserMsg.text }] });
  }
  ...
  for (const m of messages) {
    ...
    if (m.role === 'assistant' || m.role === 'ai') {
      ...
      history.push({ role: 'model', parts: toolCalls ? [{ functionCall: toolCalls[0].functionCall }] : [{ text: m.content || m.text || '' }] });
      currentRole = 'model';
    } else if (m.role === 'tool') {
      history.push({
        role: 'function',
        parts: [{ functionResponse: { name: 'unknown', response: JSON.parse(m.content) } }], // Name will be fixed later
      });
      currentRole = 'function';
    } else if (m.role === 'user' && currentRole !== 'user') {
        // Only add user message if the last message wasn't also a user message
        history.push({ role: 'user', parts: [{ text: m.content || m.text }] });
        currentRole = 'user';
    }
  }

  // Gemini requires a specific alternating order. This loop fixes tool call names
  // and ensures the function response corresponds to the right call.
...
// --- REWRITTEN ROUTER WITH AGENT LOOP ---
router.post("/travel", async (req, res) => {
  const reqId = newReqId();
  try {
    const { messages = [], userId = "anonymous" } = req.body || {};
    logInfo(reqId, `POST /chat/travel, user=${userId}, Gemini=${hasGeminiKey}, SerpApi=${hasSerpApiKey}`);
    ...
    if(convo.length > 1) {
        convo.splice(1, 0, { role: 'model', parts: [{ text: 'Understood. I will follow all instructions. How can I help?' }] });
    }
    ...
    for (let i = 0; i < MAX_TURNS; i++) {
      ...
      const toolCalls = choice?.finishReason === 'TOOL_CODE' || assistantMessage?.parts.some(p => p.functionCall)
        ? assistantMessage.parts.filter(p => p.functionCall).map(p => p.functionCall)
        : null;

      if (!toolCalls) {
        // No tool call, means it's a final text response (or an error)
        const textResponse = assistantMessage?.parts.map(p => p.text).join('') || "I'm sorry, I couldn't process that. Could you try again?";
        return res.json({ aiText: textResponse });
      }

      // It's a tool call, process it
      const toolCall = toolCalls[0]; // Process one tool at a time for simplicity
      ...
      if (functionName === "request_dates") {
        return res.json({ aiText: "When would you like to travel?", signal: { type: "dateNeeded" }, assistantMessage: { role: 'assistant', tool_calls: [{ id: `call_${reqId}`, type: 'function', function: { name: 'request_dates', arguments: '{}' } }] } });
      }
      if (functionName === "request_guests") {
        return res.json({ aiText: "How many people will be traveling?", signal: { type: "guestsNeeded" }, assistantMessage: { role: 'assistant', tool_calls: [{ id: `call_${reqId}`, type: 'function', function: { name: 'request_guests', arguments: '{}' } }] } });
      }
      ...
      // Add the function's result to history and continue the loop
      convo.push({
          role: 'function',
