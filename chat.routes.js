...
try { if (typeof globalThis.fetch !== "function") { const nodeFetch = (await import("node-fetch")).default; globalThis.fetch = nodeFetch; FETCH_SOURCE = "node-fetch"; }
} catch (e) { console.error("[chat] fetch polyfill load failed:", e?.message); }
...
const getMem = (userId) => {
  if (!userMem.has(userId)) { userMem.set(userId, { lastDest: null, lastPhotos: [] }); }
...
// ✅ FIX: Using a much more reliable fallback image URL
...
async function pickPhoto(mem, dest, reqId) {
    ...
    return FALLBACK_IMAGE_URL; // Return the reliable fallback
}

const tools = [ /* ... (rest of tools definition is unchanged) ... */ ];
const SYSTEM = `/* ... (system prompt is unchanged) ... */`;
...
router.post("/travel", async (req, res) => {
  ...
  try {
    ...
    if (!hasKey) return res.json(await runSlotFlow());
    
    // Fallback to slot flow if OpenAI logic is simplified
